-- lean_project/AbsSat/GraphPath/Model/AggressiveReview.lean
...
namespace AbsSat.GraphPath.Model.AggressiveReview
...
/-- Pins, the review `R`, then `up`. -/
def upFilteringR (R : GPathM → GPathM) (g : GPathM) (reqs : List NodeId) (d : NodeId)
    (title : String) : GPathM :=
  up (filterAllR R g reqs) d title

theorem filterAllR_review (g : GPathM) (reqs : List NodeId) :
    filterAllR review g reqs = filterAll g reqs := rfl

theorem upFilteringR_review (g : GPathM) (reqs : List NodeId) (d : NodeId) (title : String) :
    upFilteringR review g reqs d title = upFiltering g reqs d title := rfl
...
theorem ChainSound_upFilteringR (R : GPathM → GPathM) [ReviewOk R] (g : GPathM)
    ...
    (hreqs : ∀ req ∈ reqs, 0 ≤ req.step → req.step < g.current_step → (sel req.step).id = req) :
    ChainSound (upFilteringR R g reqs d title) (extend (filterAllR R g reqs) d sel) := by
  have hshape : upFilteringR R g reqs d title = addNode (filterAllR R g reqs) d title := by
    simp only [upFilteringR, up, hvalid, if_pos]
  rw [hshape]
  exact ChainSound_addNode _ d title hd hbelow hmok sel (ChainSound_filterAllR R g reqs sel h hreqs)

/-- info: 'AbsSat.GraphPath.Model.AggressiveReview.pruned_filterAllAgg' depends on axioms: [propext, Quot.sound] -/
#guard_msgs in
#print axioms pruned_filterAllAgg

/-- info: 'AbsSat.GraphPath.Model.AggressiveReview.ChainSound_filterAllAgg' depends on axioms: [propext, Quot.sound] -/
#guard_msgs in
#print axioms ChainSound_filterAllAgg

end AbsSat.GraphPath.Model.AggressiveReview
