...
/-- A chain through `d`, re-verified from the definitions before being
believed. -/
def chainThrough (g : GPathM) (d : PathNodeId) : Option (List PathNodeId) :=
  match searchFrom g d 0 [] with
  | some sel => if isGoodChain g sel then some sel else none
  | none => none

/-- Nodes of `g` that lie on no complete co-owned chain. Non-empty means the
"no zombies" invariant just failed on a real map. -/
def zombiesOf (g : GPathM) : List PathNodeId :=
  g.nodes.filterMap (fun n => if (chainThrough g n.id).isNone then some n.id else none)

/-- Walk the machine over a real map, checking every state.
Returns `(states checked, nodes checked, zombie states, zombie nodes)`. -/
partial def walk (gmap : GMap) (line : MirrorLine) (fuel : Nat)
    (acc : Nat × Nat × Nat × Nat) : Nat × Nat × Nat × Nat :=
  if fuel = 0 || line.isEmpty then acc
  else
    let acc := line.foldl (fun (a : Nat × Nat × Nat × Nat) kv =>
      let g := kv.2
      if !isValid g then a else
        let zs := zombiesOf g
        (a.1 + 1, a.2.1 + g.nodes.length,
         a.2.2.1 + (if zs.isEmpty then 0 else 1), a.2.2.2 + zs.length)) acc
    walk gmap (mirrorAdvance gmap line) (fuel - 1) acc

def report (path : String) : IO Bool := do
  let gmap ← load_import! path
  let (states, nodes, zstates, znodes) :=
    walk gmap (mirrorInit gmap) 1000 (0, 0, 0, 0)
  IO.println s!"{path}"
  IO.println s!"  valid states checked={states}  nodes checked={nodes}"
  if znodes == 0 then
    IO.println s!"  no zombies: every node lies on a verified co-owned chain ✅"
    return true
  else
...
`(seed, case)` pair. -/
def checkCnfText (cnf : String) (tmp : String) : IO (Bool × Nat × Nat) := do
  IO.FS.writeFile tmp cnf
  let gmap ← load_import! tmp
  let (states, nodes, _, znodes) := walk gmap (mirrorInit gmap) 1000 (0, 0, 0, 0)
  pure (znodes == 0, states, nodes)
...
vars={nvMin}..{nvMin + nvSpan - 1} ---"
  ...
  for idx in [0:cases] do
    ...
    rng := rng3
    let (ok, st, nd) ← checkCnfText cnf "validate_tmp.cnf"
    states := states + st
    nodes := nodes + nd
    if !ok then
      failures := failures + 1
...
valid states={states} nodes checked={nodes} ---"
  if failures == 0 then
    IO.println "Every node of every valid state lies on a verified co-owned chain. ✅"
    pure 0
  else
