...
theorem upFilteringF_eq (g : GPathM) (d : NodeId) (title : String) :
    upFilteringF φ (Fsac φ 0) g d title
      = upFilteringWeak g (weakReqOfCnf φ d) (reqOfCnf φ d) d title := rfl

theorem sendTo_eq (g : GPathM) (next : PureLine) (d : NodeId) :
    sendToF φ (Fsac φ 0) g next d = sendToW φ g next d := rfl

theorem sendAll_eq (kv : NodeId × GPathM) (next : PureLine) :
    sendAllF φ (Fsac φ 0) kv next = sendAllW φ kv next := by
  unfold sendAllF sendAllW
  have h : sendToF φ (Fsac φ 0) kv.2 = sendToW φ kv.2 := by
    funext n d
    exact sendTo_eq φ kv.2 n d
  rw [h]

theorem advance_eq (line : PureLine) : pureAdvanceF φ (Fsac φ 0) line = pureAdvanceW φ line := by
  unfold pureAdvanceF pureAdvanceW
  have h : (fun next kv => sendAllF φ (Fsac φ 0) kv next) = (fun next kv => sendAllW φ kv next) := by
    funext next kv
    exact sendAll_eq φ kv next
  rw [h]

theorem steps_eq : ∀ (n : Nat) (line : PureLine),
    pureStepsF φ (Fsac φ 0) n line = pureStepsW φ n line
  | 0, _ => rfl
  | n + 1, line => by
    show pureStepsF φ (Fsac φ 0) n (pureAdvanceF φ (Fsac φ 0) line) = pureStepsW φ n (pureAdvanceW φ line)
    rw [advance_eq φ line, steps_eq n]

/-- **The improved run is the generic run with no conditioned pass.** -/
theorem run_eq : pureRunF φ (Fsac φ 0) = pureRunW φ := steps_eq φ _ _

theorem alongF_of_alongW (g : GPathM) (h : AlongAssignW φ a g) :
    AlongAssignF φ a (Fsac φ 0) g := by
  induction h with
...
theorem alongW_of_alongF (g : GPathM) (h : AlongAssignF φ a (Fsac φ 0) g) :
...
theorem isValid_alongW (hwf : WF φ) (hsat : Sat a φ) (g : GPathM) (h : AlongAssignW φ a g) :
    isValid g = true :=
  isValid_alongSac φ a hwf hsat 0 g (alongF_of_alongW φ a g h)

theorem inhabited_alongW (hwf : WF φ) (hsat : Sat a φ) (g : GPathM) (h : AlongAssignW φ a g) :
    AbsSat.GraphPath.Model.Inhabited g :=
  inhabitedM_alongF φ a (Fsac φ 0) hwf (prunes_Fsac φ 0) (keepsBranch_Fsac φ a hsat 0) g
...
theorem pureRunW_carries (hwf : WF φ) (hsat : Sat a φ) :
  ...
  obtain ⟨g, hmem, hal, hcs⟩ :=
    pureRunF_carries φ a (Fsac φ 0) hwf hsat (prunes_Fsac φ 0) (keepsBranch_Fsac φ a hsat 0)
...
theorem pureRunW_full_state (hwf : WF φ) (hsat : Sat a φ) :
  ...
  obtain ⟨g, hmem, hcs, hv, hi⟩ := pureRunSac_full_state φ a hwf hsat 0
...
theorem pureRunW_ne_nil (hwf : WF φ) (h : Satisfiable φ) : pureRunW φ ≠ [] := by
  have hne := pureRunSac_ne_nil φ hwf 0 h
  rw [pureRunSac, run_eq φ] at hne
  exact hne
