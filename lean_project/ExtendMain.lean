...
def main (args : List String) : IO UInt32 := do
  ...
  | "--finalthread" :: rest =>
    ...
    let nvMin := (rest[2]?.bind (·.toNat?)).getD 3
    let nvSpan := (rest[3]?.bind (·.toNat?)).getD 3
    let budget := (rest[4]?.bind (·.toNat?)).getD 40000
    AbsSat.GraphPath.Model.ExtendSearch.runRandomFinalThread cases seed nvMin nvSpan budget
  | "--finalowners" :: rest =>
    let cases := (rest[0]?.bind (·.toNat?)).getD 20
    let seed := (rest[1]?.bind (·.toNat?)).getD 2026
    let nvMin := (rest[2]?.bind (·.toNat?)).getD 3
  ...
  | "--randomupdown" :: rest =>
    ...
    AbsSat.GraphPath.Model.ExtendSearch.runRandomUD cases seed nvMin nvSpan
  | "--randomclique" :: rest =>
