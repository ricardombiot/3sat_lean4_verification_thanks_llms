...
Phases F1–F6 and lemma L1 are complete and axiom-free (see the summary above). Phase F2 — including F2.c in both its fixpoint and per-node forms — lives in `AbsSat/GraphPath/Model/Fuel.lean`. Bridge lemma L2's soundness and narrowing directions live in `AbsSat/GraphPath/Model/Filter.lean`; L4's monotone direction and L6's join case in `Model/Join.lean`; L6's statement, seed case and executable falsifier (`lake exe l6search`) in `Model/L6.lean` and `Model/L6Search.lean`. L6 itself is open, and with it L2's ⊇ direction. Execution log: [../docs/plans/espejo_gpathm_lema_L1.md](../docs/plans/espejo_gpathm_lema_L1.md).
