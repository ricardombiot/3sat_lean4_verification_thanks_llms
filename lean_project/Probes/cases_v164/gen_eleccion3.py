...
roles=['A','B','C','s1','s2','s3','t1','t2','t3','y','z']
CL=[[('A',False),('s1',True),('y',True)],[('y',False),('s2',True),('s3',True)],
    [('B',True),('t1',True),('z',True)],[('z',False),('t2',True),('t3',True)]]
for i in (1,2,3):
  for j in (1,2,3):
...
def run(tag,order,corder):
    ...
    f=f'{S}/wide_{tag}.cnf'; open(f,'w').write('\n'.join(lines)+'\n')
