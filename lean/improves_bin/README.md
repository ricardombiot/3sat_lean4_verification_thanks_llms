...
| `ClauseWitness` | — | nuevo | la regla aprendida de la sonda Julia: un nodo de un paso de cláusula fija en su tabla las variables de los literales de su ventana (`req_in_table`, `parent_req_in_table`, `owns_window_req`); **`witness_fixes_clique`**: un testigo en `L_p` fija el valor de todo miembro de la clique en las variables de los literales `p` y `p-1`; `clauseKey_trueVar` (la clique contiene un valor que hace cierto un literal) y `clauseKey_allFalse` (contiene los tres valores falsos: no hay testigo en `L2`) |
