...
to `run_pure`. Theorems 02/03/06/08 follow: completeness holds for every `Bounded`
formula; soundness assumes the open hypothesis `ClauseStepExact`.
-/

namespace AbsSatBin.SatMachine.PureProofs

open AbsSatBin.Cnf
open AbsSatBin.GraphMap.CnfMapBin
open AbsSatBin.GraphPath.Model.PureDriver
open AbsSatBin.GraphPath.Model.Decision
open AbsSatBin.GraphPath.Model.NodeInvariant
open AbsSatBin.SatMachine.PureSatMachine
...
hypothesis. -/
theorem completeness_pure (cnf : Cnf) (hwf : Bounded cnf) (h : Satisfiable cnf) :
...
theorem soundness_pure (cnf : Cnf) (hwf : Bounded cnf) (hexact : ClauseStepExact cnf)
...
theorem run_pure_decides (cnf : Cnf) (hwf : Bounded cnf) (hexact : ClauseStepExact cnf) :
    is_satisfiable (run_pure cnf) = true ↔ Satisfiable cnf :=
  ⟨soundness_pure cnf hwf hexact, completeness_pure cnf hwf⟩

end AbsSatBin.SatMachine.PureProofs
