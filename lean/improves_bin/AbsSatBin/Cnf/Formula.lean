-- lean/improves_bin/AbsSatBin/Cnf/Formula.lean
...
* **`Lit.v` is 0-based.** The bin map puts variable `v` at step `2v+1` and its
  negation at `2v+2` (step 0 is the root fusion node), so a 0-based index makes
  every step an affine expression with no truncated `Nat` subtraction anywhere.
  DIMACS's 1-based numbering is converted once, at the parser boundary.
...
/-- The bin map step a literal points at: `2v+1` for a positive literal, `2v+2`
...
to step `2v+1`, `¬x` to `2v+2`). What it excludes is a repeated literal of the
same polarity, which `MapReqs.repeated_literal_not_functional` shows the
encoding genuinely cannot represent.

**In the bin map the distinct-steps half is no longer needed for the map**:
every node carries at most one requirement, so functionality is free, and a
repeated literal simply occupies two clause steps that require the same node.
`CnfMapBin` asks only for the bound half (`Bounded`). The full `WF` is kept
...
/-- The bound half of `Clause.WF`: every literal names a variable of the formula.
This is all the bin map needs. -/
def Clause.Bounded (n : Nat) (c : Clause) : Prop :=
  c.l1.v < n ∧ c.l2.v < n ∧ c.l3.v < n

def Bounded (φ : Cnf) : Prop := ∀ c ∈ φ.clauses, Clause.Bounded φ.nVars c

theorem bounded_of_wf (φ : Cnf) (h : WF φ) : Bounded φ := fun c hc => (h c hc).1
...
/-- info: 'AbsSatBin.Cnf.satB_iff' depends on axioms: [propext, Quot.sound] -/
#guard_msgs in
#print axioms satB_iff

/-- info: 'AbsSatBin.Cnf.sat_congr_below' depends on axioms: [propext] -/
#guard_msgs in
