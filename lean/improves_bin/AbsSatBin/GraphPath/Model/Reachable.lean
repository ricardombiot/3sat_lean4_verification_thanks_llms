-- lean/improves_bin/AbsSatBin/GraphPath/Model/Reachable.lean
...
by `reqOf : NodeId → List NodeId` and — **rewritten for the bin map** — by the prohibited
windows `forb : PathNodeId → Bool` the UP skips (`CnfMapBin.isProhibited φ`).
...
inductive Reachable : GPathM → Prop where
  ...
  | up (g : GPathM) (d : NodeId) (title : String)
      ...
      Reachable g → Reachable (GPathM.upFiltering g (reqOf d) d title forb)
