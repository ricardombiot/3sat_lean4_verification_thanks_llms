-- lean/improves_bin/AbsSatBin/GraphPath/Model/L6Search.lean
...
def mkReqs (width : Nat) (seed : Nat) (d : NodeId) : List NodeId :=
  (List.range d.step.toNat).filterMap (fun k =>
    ...
    if h % 2 == 0 then none  -- idx: hash of a synthetic map, not a step
...
def advance (m : SynMap) (k : Nat) (line : Line) : Line :=
  line.foldl (fun next kv =>
    (List.range m.width).foldl (fun next (i : Nat) =>
      ...
      let g' := upFiltering kv.2 (m.reqs dest) dest "n" noForb  -- synthetic maps: no windows
...
namespace L6Search
open AbsSatBin.Utils.Alias AbsSatBin.GraphPath.Model AbsSatBin.GraphPath.Model.GPathM
