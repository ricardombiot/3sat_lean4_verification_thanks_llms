-- lean/improves_bin/AbsSatBin/GraphPath/Model/Parents.lean
...
namespace AbsSatBin.GraphPath.Model.Parents

open AbsSatBin.Utils.Alias
open AbsSatBin.GraphPath.Model
open AbsSatBin.GraphPath.Model.GPathM
open AbsSatBin.GraphPath.Model.GownersNodes (HasNode hasNode_iff)
...
theorem PN_addNode (g : GPathM) (d : NodeId) (title : String) (forb : PathNodeId → Bool) (h : PN g) :
    PN (addNode g d title forb) := by
  intro n' hn' p hp
  rw [addNode_nodes] at hn'
  show ∃ m ∈ (addNode g d title forb).nodes, m.id = p
  rw [addNode_nodes]
...
theorem PN_up (g : GPathM) (d : NodeId) (title : String) (forb : PathNodeId → Bool) (h : PN g) :
    PN (up g d title forb) := by
  simp only [GPathM.up]
  split
  · split
    · exact PN_review _ (PN_addNode g d title forb h)
    · exact PN_addNode g d title forb h
  · exact h

theorem PN_upFiltering (g : GPathM) (reqs : List NodeId) (d : NodeId) (title : String) (forb : PathNodeId → Bool)
    (h : PN g) : PN (upFiltering g reqs d title forb) :=
  PN_up _ d title forb (PN_filterAll g reqs h)
...
def PBelow (h : GPathM) : Prop :=
  ∀ n ∈ h.nodes, ∀ p ∈ n.parents, p.id.step = n.id.id.step - 1  -- idx: adjacent rows (one gpath row per map step, bin map too)
...
theorem PBelow_addNode (g : GPathM) (d : NodeId) (title : String) (forb : PathNodeId → Bool)
    (hd : d.step = g.current_step) (h : PBelow g) : PBelow (addNode g d title forb) := by
  intro n' hn' p hp
  rw [addNode_nodes] at hn'
  rcases List.mem_append.mp hn' with hmem | hmem
  · obtain ⟨n, hn, hEq⟩ := List.mem_map.mp hmem
  ...
  · obtain ⟨pid, hpid, rfl⟩ := (mem_newRow_iff g d title forb n').mp hmem
    ...
    · have hstep := mem_line_step g (g.current_step - 1) p hp'
      ...
      show p.id.step = d.step - 1  -- idx: adjacent rows (one gpath row per map step, bin map too)
      rw [hstep, hd]
    · exact absurd hp' List.not_mem_nil

theorem NotRoot_addNode (g : GPathM) (d : NodeId) (title : String) (forb : PathNodeId → Bool)
    (hd : d.step = g.current_step) (_hmok : MachineOk g) (h : NotRoot g) :
    NotRoot (addNode g d title forb) := by
  intro n' hn' hpos
  rw [addNode_nodes] at hn'
  rcases List.mem_append.mp hn' with hmem | hmem
  · obtain ⟨n, hn, hEq⟩ := List.mem_map.mp hmem
  ...
  · obtain ⟨pid, hpid, rfl⟩ := (mem_newRow_iff g d title forb n').mp hmem
    ...
    refine parent_id_ne_none_of_mem_newRowIds g d forb pid ?_ hpid
    rw [mapId_of_mem_newRowIds g d forb pid hpid] at hpos
    omega
...
theorem Shape_upFiltering (g : GPathM) (d : NodeId) (title : String)
    ...
    Shape (upFiltering g (reqOf d) d title forb) := by
  ...
  show Shape (up (filterAll g (reqOf d)) d title forb)
  simp only [GPathM.up]
  have hadd : Shape (addNode (filterAll g (reqOf d)) d title forb) :=
    ⟨PN_addNode _ d title forb hf.pn, PBelow_addNode _ d title forb hd hf.pbelow,
      NotRoot_addNode _ d title forb hd hmokf hf.notroot⟩
  split
  · split
    · exact Shape_of_pruned_pn (pruned_review _) (PN_review _ hadd.pn) hadd
    · exact hadd
  · exact hf

/-- **The three shape invariants, of every state the machine builds.** -/
theorem Shape_reachable (g : GPathM) (h : Reachable reqOf forb g) : Shape g := by
  induction h with
  ...
  | up g d title hstep _ _ hr ih =>
    exact Shape_upFiltering reqOf forb g d title hstep
      (Certifies.MachineOk_reachable reqOf forb g hr) ih
...
theorem Shape_filterAll (g : GPathM) (reqs : List NodeId) (h : Reachable reqOf forb g) :
    Shape (filterAll g reqs) :=
  Shape_of_pruned_pn (pruned_filterAll g reqs)
    (PN_filterAll g reqs (Shape_reachable reqOf forb g h).pn) (Shape_reachable reqOf forb g h)

/-- info: 'AbsSatBin.GraphPath.Model.Parents.Shape_reachable' depends on axioms: [propext, Quot.sound] -/
#guard_msgs in
#print axioms Shape_reachable

end AbsSatBin.GraphPath.Model.Parents
