-- lean/improves_bin/AbsSatBin/GraphPath/Model/PureDriver.lean
import AbsSatBin.GraphPath.Model.Conservation

/-!
# The driver, over the arithmetic bin map — **revised** (`lean_project`'s `PureDriver`)

Bin map changes: destinations are `CnfMapBin.sonsOfMap` (a positive variable node is sent only
to the negation node it agrees with, as Julia's `map_node.sons`), requirements are
`CnfMapBin.reqOf`, and the UP skips `isProhibited φ`. `pureRun_ne_nil` needs `Bounded` only.
...
So this is the same loop over `CnfMapBin`/`CnfSelBin`: `sonsOfMap` in place of
`map_node.sons`, `reqOf` in place of `destine_node.requires`, `mapNodes` in
...
namespace AbsSatBin.GraphPath.Model.PureDriver
...
def sendTo (φ : Cnf) (g : GPathM) (next : PureLine) (d : NodeId) : PureLine :=
  if isValid (upFiltering g (reqOf φ d) d "" (isProhibited φ)) then
    insertPure next d (upFiltering g (reqOf φ d) d "" (isProhibited φ))
  else next
...
destination is among `sonsOfMap`, so the inner fold visits it; the result is
`AlongAssign`, so the conservation law still applies to it; and `isValid` holds,
so the `if` takes the inserting branch. -/
theorem advance_target (φ : Cnf) (a : Assign) (hwf : Bounded φ) (hsat : Sat a φ)
...
/-- info: 'AbsSatBin.GraphPath.Model.PureDriver.advance_target' depends on axioms: [propext, Quot.sound] -/
#guard_msgs in
...
theorem isValid_filterAll_of_sent (φ : Cnf) (g : GPathM) (d : NodeId)
    (hval : isValid (upFiltering g (reqOf φ d) d "" (isProhibited φ)) = true) :
    isValid (filterAll g (reqOf φ d)) = true := by
  by_cases h : isValid (filterAll g (reqOf φ d)) = true
  · exact h
  · exfalso
    have he : upFiltering g (reqOf φ d) d "" (isProhibited φ) = filterAll g (reqOf φ d) := by
...
theorem StateOk_sent (φ : Cnf) (k : Int) (kv : NodeId × GPathM) (hkv : StateOk φ k kv)
    ...
    (hval : isValid (upFiltering kv.2 (reqOf φ d) d "" (isProhibited φ)) = true) :
    StateOk φ (k + 1) (d, upFiltering kv.2 (reqOf φ d) d "" (isProhibited φ)) := by
  have hkey : kv.1.step = k := mapNodes_step φ k kv.1 hkv.onMap
  have hd' : d ∈ mapNodes φ (k + 1) := by
    have := sonsOfMap_subset φ kv.1 (by rw [hkey]; exact hkv.onMap) d hd
    rwa [hkey] at this
  have hdstep : d.step = k + 1 := mapNodes_step φ (k + 1) d hd'
  have hfv := isValid_filterAll_of_sent φ kv.2 d hval
  -- the UP sets `map_parent`, and a review after a skipped window keeps it
  have hmp : (upFiltering kv.2 (reqOf φ d) d "" (isProhibited φ)).map_parent = some d := by
    simp only [upFiltering, GPathM.up, hfv, if_true]
    split
    · rw [(pruned_review _).map_parent_eq]; rfl
    · rfl
  refine ⟨hd', ?_, ?_, hmp, hval⟩
  · exact MapReachable.up kv.2 d "" (by rw [hdstep, hkv.step]) (by rw [hdstep]; exact hd') hkv.reach
  · rw [Conservation.current_step_upFiltering _ _ _ _ _ hfv, hkv.step]
...
theorem LineOk_sendTo (φ : Cnf) (k : Int) (kv : NodeId × GPathM) (hkv : StateOk φ k kv)
    (d : NodeId) (hd : d ∈ sonsOfMap φ kv.1) (next : PureLine)
...
theorem Carries_sendTo (φ : Cnf) (a : Assign) (k : Int) (kv : NodeId × GPathM)
    (hkv : StateOk φ k kv) (d : NodeId) (hd : d ∈ sonsOfMap φ kv.1)
...
theorem sons_fold_mono (φ : Cnf) (a : Assign) (k : Int) (kv : NodeId × GPathM)
    (hkv : StateOk φ k kv) :
    ∀ (l : List NodeId), (∀ d ∈ l, d ∈ sonsOfMap φ kv.1) →
...
theorem sons_fold_establish (φ : Cnf) (a : Assign) (hwf : Bounded φ) (hsat : Sat a φ)
    (hzero : (0 : Int) < stepCount φ) (k : Int) (h0 : 0 ≤ k) (hk : k + 1 < stepCount φ)
    (g : GPathM) (hal : AlongAssign φ a g) (hcs : g.current_step = k + 1)
...
theorem Carries_pureAdvance (φ : Cnf) (a : Assign) (hwf : Bounded φ) (hsat : Sat a φ)
    (hzero : (0 : Int) < stepCount φ) (k : Int) (h0 : 0 ≤ k) (hk : k + 1 < stepCount φ)
...
/-- info: 'AbsSatBin.GraphPath.Model.PureDriver.Carries_pureAdvance' depends on axioms: [propext, Quot.sound] -/
#guard_msgs in
...
theorem run_ok (φ : Cnf) (a : Assign) (hwf : Bounded φ) (hsat : Sat a φ)
...
destroy it. -/
theorem pureRun_carries (φ : Cnf) (a : Assign) (hwf : Bounded φ) (hsat : Sat a φ)
    (hzero : (0 : Int) < stepCount φ) :
...
theorem pureRun_ne_nil (φ : Cnf) (hwf : Bounded φ) (hzero : (0 : Int) < stepCount φ)
...
theorem pureRun_full_state (φ : Cnf) (a : Assign) (hwf : Bounded φ) (hsat : Sat a φ)
    (hzero : (0 : Int) < stepCount φ) :
    ∃ g, (selOfAssign φ a (stepCount φ - 1), g) ∈ pureRun φ
...
/-- info: 'AbsSatBin.GraphPath.Model.PureDriver.init_ok' depends on axioms: [propext, Quot.sound] -/
#guard_msgs in
#print axioms init_ok

/-- info: 'AbsSatBin.GraphPath.Model.PureDriver.run_ok' depends on axioms: [propext, Quot.sound] -/
#guard_msgs in
#print axioms run_ok

/-- info: 'AbsSatBin.GraphPath.Model.PureDriver.pureRun_carries' depends on axioms: [propext, Quot.sound] -/
#guard_msgs in
#print axioms pureRun_carries

/-- info: 'AbsSatBin.GraphPath.Model.PureDriver.pureRun_full_state' depends on axioms: [propext, Quot.sound] -/
#guard_msgs in
#print axioms pureRun_full_state

/-- info: 'AbsSatBin.GraphPath.Model.PureDriver.pureRun_ne_nil' depends on axioms: [propext, Quot.sound] -/
#guard_msgs in
...
end AbsSatBin.GraphPath.Model.PureDriver
