-- lean/improves_bin/AbsSatBin/GraphPath/Model/ArcConsistency.lean
...
theorem review_arcConsistent (reqOf : NodeId → List NodeId) (forb : PathNodeId → Bool) (g : GPathM)
    (h : isValid (review g) = true) (hreach : Reachable reqOf forb (review g)) :
    ArcConsistent reqOf (review g) where
  supported := review_arcSupported g h
  pinned := L1 reqOf forb hreach
...
/-- info: 'AbsSatBin.GraphPath.Model.review_arcConsistent' depends on axioms: [propext, Quot.sound] -/
#guard_msgs in
#print axioms review_arcConsistent

/-- info: 'AbsSatBin.GraphPath.Model.review_arcSupported' depends on axioms: [propext, Quot.sound] -/
#guard_msgs in
