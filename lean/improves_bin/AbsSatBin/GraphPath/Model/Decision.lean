-- lean/improves_bin/AbsSatBin/GraphPath/Model/Decision.lean
...
/-!
# The machine as a decision procedure, modulo the clause filter — **revised** for the bin map

On the bin map `ClauseStepExact` bundles two obligations (`NodeInvariant`): the filter at the
literal steps of the clauses, and the review after a skipped window (`SkipExact`).
...
namespace AbsSatBin.GraphPath.Model.Decision
...
theorem decides_of_ClauseStepExact (φ : Cnf) (hwf : Bounded φ) (hzero : (0 : Int) < stepCount φ)
    (h : ClauseStepExact φ) :
  ...
  · rintro ⟨kv, hmem, hv⟩
    ...
    obtain ⟨sel, hsel⟩ := chain_of_SupportedS kv.2
      (GownersNodes.GN_reachable (reqOf φ) (isProhibited φ) kv.2 hreach) hsup hv
      (pos_reachable (reqOf φ) (isProhibited φ) kv.2 hreach)
...
/-- info: 'AbsSatBin.GraphPath.Model.Decision.decides_of_ClauseStepExact' depends on axioms: [propext, Quot.sound] -/
#guard_msgs in
#print axioms decides_of_ClauseStepExact

end AbsSatBin.GraphPath.Model.Decision
