-- lean/improves_bin/AbsSatBin/GraphPath/Model/SelfOwn.lean
...
namespace AbsSatBin.GraphPath.Model.SelfOwn
...
theorem SNN_addNode (g : GPathM) (d : NodeId) (title : String) (forb : PathNodeId → Bool)
    (hd : d.step = g.current_step) (hmok : MachineOk g) (h : SNN g) :
    SNN (addNode g d title forb) := by
  intro n' hn'
...
theorem SNN_reachable (g : GPathM) (h : Reachable reqOf forb g) : SNN g := by
  induction h with
  | seed d title hstep _ => exact SNN_initSeed d title hstep
  | up g d title hstep _ _ hr ih =>
    have hpr := pruned_filterAll g (reqOf d)
    show SNN (up (filterAll g (reqOf d)) d title forb)
    simp only [GPathM.up]
    split
    · split
      · exact SNN_of_pruned (pruned_review _) (SNN_addNode _ d title forb (by rw [hpr.step_eq]; exact hstep)
            (MachineOk_of_pruned hpr (Certifies.MachineOk_reachable reqOf forb g hr))
            (SNN_of_pruned hpr ih))
      · exact SNN_addNode _ d title forb (by rw [hpr.step_eq]; exact hstep)
            (MachineOk_of_pruned hpr (Certifies.MachineOk_reachable reqOf forb g hr))
            (SNN_of_pruned hpr ih)
...
theorem OOS_addNode (g : GPathM) (d : NodeId) (title : String) (forb : PathNodeId → Bool)
    (hd : d.step = g.current_step)
    (hbelow : ∀ n ∈ g.nodes, n.id.id.step < g.current_step)
    (hgn : GownersNodes.GN g) (h : OOS g) : OOS (addNode g d title forb) := by
  intro n' hn' q hq hstep
  rw [addNode_nodes] at hn'
  rcases List.mem_append.mp hn' with hmem | hmem
  · obtain ⟨n, hn, hEq⟩ := List.mem_map.mp hmem
    have hni : n'.id = n.id := by rw [← hEq]; exact upMap_id g d forb n
    have hno : n'.owners = n.owners ++ gainedOwners g d forb n := by
      rw [← hEq]; exact upMap_owners g d forb n
    ...
    · exfalso
      have h1 : q.id.step = d.step := by
        rw [mapId_of_mem_newRowIds g d forb q (gainedOwners_subset g d forb n q hq)]
      have h2 := hbelow n hn
      rw [h1, hd] at hstep
      omega
  · obtain ⟨pid, hpid, rfl⟩ := (mem_newRow_iff g d title forb n').mp hmem
    ...
    · exfalso
      ...
      have h2 := hbelow m hm
      rw [hmid] at h2
      rw [mapId_of_mem_newRowIds g d forb pid hpid, hd] at hstep
      omega
    · rfl
...
theorem Below_addNode (g : GPathM) (d : NodeId) (title : String) (forb : PathNodeId → Bool)
    ...
    Below (addNode g d title forb) := by
  intro n' hn'
  rw [addNode_current]
  rw [addNode_nodes] at hn'
  rcases List.mem_append.mp hn' with hmem | hmem
...
theorem OwnBelow_addNode (g : GPathM) (d : NodeId) (title : String) (forb : PathNodeId → Bool)
    (hd : d.step = g.current_step) (h : OwnBelow g) : OwnBelow (addNode g d title forb) := by
  intro n' hn' q hq
  rw [addNode_current]
  rw [addNode_nodes] at hn'
  rcases List.mem_append.mp hn' with hmem | hmem
  · obtain ⟨n, hn, hEq⟩ := List.mem_map.mp hmem
    ...
    · rw [mapId_of_mem_newRowIds g d forb q (gainedOwners_subset g d forb n q hq), hd]; omega
  · obtain ⟨pid, hpid, rfl⟩ := (mem_newRow_iff g d title forb n').mp hmem
    rw [rowNode_owners] at hq
    ...
    · obtain ⟨p, _, mp, hmp, hqmp⟩ := exists_owner_of_mem_unionOwnersOf g _ q hinh
      ...
      omega
    · rw [mapId_of_mem_newRowIds g d forb q hpid, hd]; omega

theorem OwnBelow_up (g : GPathM) (d : NodeId) (title : String) (forb : PathNodeId → Bool)
    (hd : d.step = g.current_step) (h : OwnBelow g) : OwnBelow (up g d title forb) := by
  simp only [GPathM.up]
  split
  · split
    · exact OwnBelow_of_pruned (pruned_review _) (OwnBelow_addNode g d title forb hd h)
    · exact OwnBelow_addNode g d title forb hd h
  · exact h
...
theorem OwnBelow_reachable (g : GPathM) (h : Reachable reqOf forb g) : OwnBelow g := by
  induction h with
  | seed d title hstep _ => exact OwnBelow_initSeed d title hstep
  | up g d title hstep _ _ hr ih =>
    have hpr := pruned_filterAll g (reqOf d)
    show OwnBelow (up (filterAll g (reqOf d)) d title forb)
    simp only [GPathM.up]
    split
    · split
      · exact OwnBelow_of_pruned (pruned_review _) (OwnBelow_addNode _ d title forb (by rw [hpr.step_eq]; exact hstep)
            (OwnBelow_filterAll g (reqOf d) ih))
      · exact OwnBelow_addNode _ d title forb (by rw [hpr.step_eq]; exact hstep)
            (OwnBelow_filterAll g (reqOf d) ih)
    · exact OwnBelow_filterAll g (reqOf d) ih
  | join g₁ g₂ hok _ _ ih₁ ih₂ => exact OwnBelow_join g₁ g₂ hok ih₁ ih₂

theorem OOS_reachable (g : GPathM) (h : Reachable reqOf forb g) : OOS g := by
  induction h with
  | seed d title _ _ => exact OOS_initSeed d title
  | up g d title hstep _ _ hr ih =>
    have hpr := pruned_filterAll g (reqOf d)
    show OOS (up (filterAll g (reqOf d)) d title forb)
    simp only [GPathM.up]
    split
    · split
      · exact OOS_review _ (OOS_addNode _ d title forb (by rw [hpr.step_eq]; exact hstep)
            (Certifies.nodes_below_of_pruned hpr (steps_below_current reqOf forb hr))
            (GownersNodes.GN_filterAll g (reqOf d) (GownersNodes.GN_reachable reqOf forb g hr))
            (OOS_filterAll g (reqOf d) ih))
      · exact OOS_addNode _ d title forb (by rw [hpr.step_eq]; exact hstep)
            (Certifies.nodes_below_of_pruned hpr (steps_below_current reqOf forb hr))
            (GownersNodes.GN_filterAll g (reqOf d) (GownersNodes.GN_reachable reqOf forb g hr))
            (OOS_filterAll g (reqOf d) ih)
...
theorem SelfOwned_filterAll (g : GPathM) (reqs : List NodeId) (hreach : Reachable reqOf forb g)
    (hv : isValid (filterAll g reqs) = true) :
    Ownership.SelfOwned (filterAll g reqs) := by
  have hpr := pruned_filterAll g reqs
  exact SelfOwned_of_OOS _ hv (OOS_filterAll g reqs (OOS_reachable reqOf forb g hreach))
    (SNN_of_pruned hpr (SNN_reachable reqOf forb g hreach))
    (Certifies.nodes_below_of_pruned hpr (steps_below_current reqOf forb hreach))

/-- info: 'AbsSatBin.GraphPath.Model.SelfOwn.SelfOwned_filterAll' depends on axioms: [propext, Quot.sound] -/
#guard_msgs in
#print axioms SelfOwned_filterAll

end AbsSatBin.GraphPath.Model.SelfOwn
