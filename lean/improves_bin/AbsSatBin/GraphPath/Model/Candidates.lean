-- lean/improves_bin/AbsSatBin/GraphPath/Model/Candidates.lean
...
namespace AbsSatBin.GraphPath.Model.Candidates

open AbsSatBin.Utils.Alias
open AbsSatBin.GraphPath.Model
open AbsSatBin.GraphPath.Model.GPathM
open AbsSatBin.GraphPath.Model.GownersNodes (GN GN_filterAll GN_reachable hasNode_iff)
...
theorem candidate_at_step_filterAll (g : GPathM) (reqs : List NodeId)
  ...
  candidate_at_step _ hv (GN_filterAll g reqs (GN_reachable reqOf forb g hreach)) pid n hn k hlo hhi
...
theorem domains_nonempty (g : GPathM) (reqs : List NodeId)
    ...
    (∀ pid n, (filterAll g reqs).node? pid = some n →
      ∀ k, 0 ≤ k → k < (filterAll g reqs).current_step →
        ∃ q ∈ ownersAt n.owners k,
          ((filterAll g reqs).node? q).isSome = true ∧ q.id.step = k) :=
  ⟨fun k hlo hhi => GownersNodes.node_at_every_step reqOf forb g reqs hreach hv k hlo hhi,
   fun pid n hn k hlo hhi =>
     candidate_at_step_filterAll reqOf forb g reqs hreach hv pid n hn k hlo hhi⟩
...
/-- info: 'AbsSatBin.GraphPath.Model.Candidates.candidate_at_step' depends on axioms: [propext, Quot.sound] -/
#guard_msgs in
#print axioms candidate_at_step

/-- info: 'AbsSatBin.GraphPath.Model.Candidates.domains_nonempty' depends on axioms: [propext, Quot.sound] -/
#guard_msgs in
#print axioms domains_nonempty

end AbsSatBin.GraphPath.Model.Candidates
