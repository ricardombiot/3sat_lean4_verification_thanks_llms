-- lean/improves_bin/AbsSatBin/GraphPath/Model/OwnersInvariants.lean
...
private theorem initSeed_nodes (d : NodeId) (title : String) :
  ...
  simp [GPathM.isValid, GPathM.empty, GPathM.intRange, GPathM.hasStepEntry,
    GPathM.newRow, GPathM.newRowIds, GPathM.shiftRowIds, GPathM.skipsWindow, GPathM.noForb,
    GPathM.rowNode, GPathM.rowParents, GPathM.rowOwners, GPathM.newParents, GPathM.unionOwnersOf]

private theorem initSeed_current (d : NodeId) (title : String) :
    (GPathM.initSeed d title).current_step = 1 := by
  unfold GPathM.initSeed GPathM.up GPathM.addNode
  simp [GPathM.isValid, GPathM.empty, GPathM.intRange, GPathM.hasStepEntry, GPathM.skipsWindow,
    GPathM.noForb]
...
private theorem mem_addNode {g : GPathM} {d : NodeId} {title : String} {forb : PathNodeId → Bool}
    {n' : PNodeM} (hn' : n' ∈ (GPathM.addNode g d title forb).nodes) :
    (∃ n ∈ g.nodes, n'.id = n.id ∧ n'.owners = n.owners ++ GPathM.gainedOwners g d forb n) ∨
    (∃ pid ∈ GPathM.newRowIds g d forb, n'.id = pid ∧ n'.owners = GPathM.rowOwners g d pid) := by
  ...
  · obtain ⟨pid, hpid, rfl⟩ := (GPathM.mem_newRow_iff g d title forb n').mp hnew
...
private theorem structure_addNode {g' : GPathM} (h : NodeStructure reqOf g')
    {d : NodeId} {title : String} {forb : PathNodeId → Bool} (hd : d.step = g'.current_step)
    (hback : ∀ req, req ∈ reqOf d → req.step < d.step) :
    NodeStructure reqOf (GPathM.addNode g' d title forb) := by
  intro n' hn'
  have hcs : (GPathM.addNode g' d title forb).current_step = g'.current_step + 1 := rfl
  ...
  · have hids : n'.id.id = d := by
      rw [hid]; exact GPathM.mapId_of_mem_newRowIds g' d forb pid hpid
    constructor
    · rw [hcs, hids]; omega
    · intro req hreq
      rw [hids] at hreq ⊢
...
private theorem reachable_structure (h : Reachable reqOf forb g) : NodeStructure reqOf g := by
  induction h with
  ...
  | up g d title hstep hreqs_back hreqs_distinct h_reach ih =>
    ...
    have hadd := structure_addNode reqOf (title := title) (forb := forb) hstruct'
      (hstep.trans hpr.step_eq.symm) hreqs_back
    dsimp only [GPathM.upFiltering, GPathM.up]
    split
    · split
      · exact structure_pruned reqOf (pruned_review _) hadd
      · exact hadd
    · exact hstruct'
...
theorem steps_below_current (h : Reachable reqOf forb g) :
    ∀ n ∈ g.nodes, n.id.id.step < g.current_step :=
  fun n hn => (reachable_structure reqOf forb h n hn).1

theorem reqs_back_trans (h : Reachable reqOf forb g) :
    ∀ n ∈ g.nodes, ∀ req ∈ reqOf n.id.id, req.step < n.id.id.step :=
  fun n hn => (reachable_structure reqOf forb h n hn).2
...
theorem pid_safe (h_reach : Reachable reqOf forb g) (n : PNodeM) (hn : n ∈ g.nodes)
    (pid : PathNodeId) (hpid_step : pid.id.step = g.current_step) :
    ∀ req ∈ reqOf n.id.id, pid.id.step ≠ req.step := by
  have h_back := reqs_back_trans reqOf forb h_reach n hn
  have h_below := steps_below_current reqOf forb h_reach n hn
  intro req hreq
...
private theorem addNode_ReqFiltered {g : GPathM} (h_reach : Reachable reqOf forb g)
    ...
    ReqFiltered reqOf (GPathM.addNode (GPathM.filterAll g (reqOf d)) d title forb) := by
  ...
  · -- Old node: its owners either predate the UP (invariant transfers) or
    ...
    · exfalso
      have hq' : q.id = d :=
        GPathM.mapId_of_mem_newRowIds _ d forb q (GPathM.gainedOwners_subset _ d forb m q hq)
      obtain ⟨n₀, hn₀, hid₀, _, _⟩ := hpr.nodes_derived m hm
      have hreq₀ : req ∈ reqOf n₀.id.id := by rw [← hid₀]; exact hreqm
      obtain ⟨h2, hback⟩ := reachable_structure reqOf forb h_reach n₀ hn₀
  ...
  · -- Row node: what it inherits was cleaned by `filterAll` (it is still a
    ...
    have hids : n'.id.id = d := by
      rw [hid]; exact GPathM.mapId_of_mem_newRowIds _ d forb pid hpid
    rw [hids] at hreq
    rw [hown] at hq
    ...
    · exfalso
      have hq' : q.id = d := GPathM.mapId_of_mem_newRowIds _ d forb q hpid
      have h1 : req.step < d.step := hreqs_back req hreq
      have h3 : q.id.step = d.step := by rw [hq']
      omega

theorem upFiltering_ReqFiltered (h : ReqFiltered reqOf g) (h_reach : Reachable reqOf forb g)
    (d : NodeId) (title : String) (hstep : d.step = g.current_step)
    (hreqs_back : ∀ req, req ∈ reqOf d → req.step < d.step)
    (_hreqs_distinct : ∀ r₁ r₂, r₁ ∈ reqOf d → r₂ ∈ reqOf d → r₁.step = r₂.step → r₁ = r₂) :
    ReqFiltered reqOf (GPathM.upFiltering g (reqOf d) d title forb) := by
  have h_g' : ReqFiltered reqOf (GPathM.filterAll g (reqOf d)) :=
    filterAll_preserves_ReqFiltered reqOf h (reqOf d)
  have hadd := addNode_ReqFiltered reqOf forb h_reach d title h_g' hstep hreqs_back
  dsimp only [GPathM.upFiltering, GPathM.up]
  split
  · split
    · -- a window was skipped: the review only narrows owners
      exact review_preserves_ReqFiltered reqOf hadd
    · exact hadd
  · exact h_g'
...
theorem L1 (h : Reachable reqOf forb g) : ReqFiltered reqOf g := by
  induction h with
  | seed d title hstep hreqs_back =>
    exact initSeed_ReqFiltered reqOf d title hstep hreqs_back
  | up g d title hstep hreqs_back hreqs_distinct h_reach ih =>
    exact upFiltering_ReqFiltered reqOf forb ih h_reach d title hstep hreqs_back hreqs_distinct
...
theorem L1_cor (h_reach : Reachable reqOf forb g)
  ...
  have h_inv : ReqFiltered reqOf g := L1 reqOf forb h_reach
  ...
  · -- req.step = j: contradiction via reqs_back_trans + chain_step_eq
    ...
    have h_back := reqs_back_trans reqOf forb h_reach n hn_mem req hreq_n
...
/-- info: 'AbsSatBin.GraphPath.Model.L1' depends on axioms: [propext, Quot.sound] -/
#guard_msgs in
#print axioms L1

/-- info: 'AbsSatBin.GraphPath.Model.L1_cor' depends on axioms: [propext, Quot.sound] -/
#guard_msgs in
#print axioms L1_cor
