-- lean/improves_bin/AbsSatBin/GraphPath/Model/AddNode.lean
...
theorem addNode_map_parent (g : GPathM) (d : NodeId) (title : String) (forb : PathNodeId → Bool) :
    (addNode g d title forb).map_parent = some d := rfl

theorem MachineOk_empty : MachineOk empty := by
  simp [MachineOk, empty]

theorem MachineOk_addNode (g : GPathM) (d : NodeId) (title : String) (forb : PathNodeId → Bool)
    (h : MachineOk g) : MachineOk (addNode g d title forb) := by
  obtain ⟨h0, _, _⟩ := h
...
theorem mem_upMap_sons (g : GPathM) (d : NodeId) (forb : PathNodeId → Bool) (n : PNodeM)
    (q : PathNodeId) (hq : q ∈ n.sons) : q ∈ (upMap g d forb n).sons :=
...
Bin map: unless that row node is prohibited (`hf`). -/
theorem extendPid_mem_upMap_sons (g : GPathM) (d : NodeId) (forb : PathNodeId → Bool)
    (sel : Int → PathNodeId)
    (hpos : 0 < g.current_step) (hchain : IsChain g sel) (hf : forb (extendPid g d sel) = false)
    (n : PNodeM) (hn : g.node? (sel (g.current_step - 1)) = some n) :
    extendPid g d sel ∈ (upMap g d forb n).sons := by
  rw [upMap_sons]
  refine List.mem_append_right _
    (List.mem_filter.mpr ⟨extendPid_mem_newRowIds g d forb sel hchain hf, ?_⟩)
...
theorem ChainSound_addNode (g : GPathM) (d : NodeId) (title : String) (forb : PathNodeId → Bool)
    (hd : d.step = g.current_step)
    (hbelow : ∀ n ∈ g.nodes, n.id.id.step < g.current_step)
    (hmok : MachineOk g)
    (sel : Int → PathNodeId) (h : ChainSound g sel) (hf : forb (extendPid g d sel) = false) :
    ChainSound (addNode g d title forb) (extend g d sel) := by
  obtain ⟨h0, hz, hposmp⟩ := hmok
  have hpidmem : extendPid g d sel ∈ newRowIds g d forb :=
    extendPid_mem_newRowIds g d forb sel h.chain.1 hf
  have hnew := addNode_node?_new g d title forb hd hbelow _ hpidmem
  refine ⟨ChainG_addNode g d title forb hd hbelow sel h.self_owned h.chain hf, ?_, ?_, ?_⟩
  ...
  · -- son_link: the top old node gains the new one as a son
    ...
    · rw [hk, extend_top, extend_below g d sel k (by omega)]
      ...
      subst hkeq
      simp only [sonsOf, addNode_node?_old g d title forb _ n hn]
      exact extendPid_mem_upMap_sons g d forb sel (by omega) h.chain.1 hf n hn
    · have hlt : k + 1 < g.current_step := by omega
      ...
      | some n =>
        rw [hn] at hs
        rw [addNode_node?_old g d title forb _ n hn]
        exact mem_upMap_sons g d forb n _ hs
...
**Bin map:** and whose extension is not prohibited (`hf`) — for the chain of a satisfying
assignment that is a fact about the map, proved where the map is. When a window was skipped the
UP reviews, and `ChainSound_review` carries the chain through it. -/
theorem ChainSound_upFiltering (g : GPathM) (reqs : List NodeId) (d : NodeId) (title : String)
    ...
    (hreqs : ∀ req ∈ reqs, 0 ≤ req.step → req.step < g.current_step → (sel req.step).id = req)
    (hf : forb (extendPid (filterAll g reqs) d sel) = false) :
  ...
  have hadd := ChainSound_addNode _ d title forb hd hbelow hmok sel
    (ChainSound_filterAll g reqs sel h hreqs) hf
...
/-- info: 'AbsSatBin.GraphPath.Model.ChainSound_addNode' depends on axioms: [propext, Quot.sound] -/
#guard_msgs in
#print axioms ChainSound_addNode

/-- info: 'AbsSatBin.GraphPath.Model.ChainSound_upFiltering' depends on axioms: [propext, Quot.sound] -/
#guard_msgs in
#print axioms ChainSound_upFiltering

/-- info: 'AbsSatBin.GraphPath.Model.ChainSound_initSeed' depends on axioms: [propext, Quot.sound] -/
#guard_msgs in
