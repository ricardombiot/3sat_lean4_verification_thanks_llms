-- lean/improves_bin/AbsSatBin/GraphPath/Model/GownersNodes.lean
...
namespace AbsSatBin.GraphPath.Model.GownersNodes

open AbsSatBin.Utils.Alias
open AbsSatBin.GraphPath.Model
open AbsSatBin.GraphPath.Model.GPathM
open AbsSatBin.GraphPath.Model.Certifies (GownersAreNodes)
...
theorem GN_addNode (g : GPathM) (d : NodeId) (title : String) (forb : PathNodeId → Bool)
    (h : GN g) : GN (addNode g d title forb) := by
  intro q hq
  rw [addNode_gowners, List.mem_append] at hq
  show ∃ n ∈ (addNode g d title forb).nodes, n.id = q
  rw [addNode_nodes]
  rcases hq with hq | hq
  · obtain ⟨n, hn, hid⟩ := h q hq
    exact ⟨upMap g d forb n, List.mem_append_left _ (List.mem_map_of_mem hn),
      (upMap_id g d forb n).trans hid⟩
  · exact ⟨rowNode g d title q,
      List.mem_append_right _ (List.mem_map_of_mem hq), rfl⟩

theorem GN_up (g : GPathM) (d : NodeId) (title : String) (forb : PathNodeId → Bool) (h : GN g) :
    GN (up g d title forb) := by
  simp only [GPathM.up]
  split
  · split
    · exact GN_review _ (GN_addNode g d title forb h)
    · exact GN_addNode g d title forb h
  · exact h

theorem GN_upFiltering (g : GPathM) (reqs : List NodeId) (d : NodeId) (title : String)
    (forb : PathNodeId → Bool) (h : GN g) : GN (upFiltering g reqs d title forb) :=
  GN_up _ d title forb (GN_filterAll g reqs h)
...
theorem GN_reachable (g : GPathM) (h : Reachable reqOf forb g) : GN g := by
  induction h with
  | seed d title _ _ => exact GN_initSeed d title
  | up g d title _ _ _ _ ih => exact GN_upFiltering g (reqOf d) d title forb ih
...
theorem GownersAreNodes_reachable (g : GPathM) (h : Reachable reqOf forb g) :
    GownersAreNodes g :=
  GownersAreNodes_of_GN (GN_reachable reqOf forb g h)
...
theorem GownersAreNodes_filterAll (g : GPathM) (reqs : List NodeId)
    (h : Reachable reqOf forb g) : GownersAreNodes (filterAll g reqs) :=
  GownersAreNodes_of_GN (GN_filterAll g reqs (GN_reachable reqOf forb g h))
...
theorem node_at_every_step (g : GPathM) (reqs : List NodeId) (h : Reachable reqOf forb g)
  ...
  obtain ⟨n, hn, hid⟩ := GN_filterAll g reqs (GN_reachable reqOf forb g h) q hq
  exact ⟨n, hn, by rw [hid]; exact hstep⟩

/-- info: 'AbsSatBin.GraphPath.Model.GownersNodes.GownersAreNodes_reachable' depends on axioms: [propext, Quot.sound] -/
#guard_msgs in
#print axioms GownersAreNodes_reachable

/-- info: 'AbsSatBin.GraphPath.Model.GownersNodes.node_at_every_step' depends on axioms: [propext, Quot.sound] -/
#guard_msgs in
#print axioms node_at_every_step

end AbsSatBin.GraphPath.Model.GownersNodes
