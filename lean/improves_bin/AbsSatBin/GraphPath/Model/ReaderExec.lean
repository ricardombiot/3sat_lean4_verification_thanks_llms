...
The reading program is `lean_project`'s, unchanged. It reads the final line of the bin machine
(`PureDriver.pureRun`: no weak requirements, UP with prohibited windows) instead of `pureRunW`, and
its soundness goes through `L7.sat_of_inhabited` instead of the weak-requirement route.
...
def readerVerdictW (φ : Cnf) : Bool :=
  (pureRun φ).any (fun kv => (readAgg (filterAllAgg kv.2 [])).isSome)
...
theorem readerVerdictW_sound (φ : Cnf) (hbd : Bounded φ) (h : readerVerdictW φ = true) :
  ...
  · rw [if_pos hv] at hs
    ...
    exact L7.satisfiable_of_inhabited φ hbd kv.2 hok.reach (by rw [hok.step]; omega)
      ⟨p, denot_of_pruned (pruned_filterAllAgg kv.2 []) hnd p hp⟩
...
theorem readerVerdictW_complete (φ : Cnf) (kv : NodeId × GPathM) (hkv : kv ∈ pureRun φ)
...
/-- info: 'AbsSatBin.GraphPath.Model.ReaderExec.readerVerdictW_complete' depends on axioms: [propext, Quot.sound] -/
#guard_msgs in
...
theorem readerVerdictW_of_chains (φ : Cnf) (kv : NodeId × GPathM) (hkv : kv ∈ pureRun φ)
...
/-- info: 'AbsSatBin.GraphPath.Model.ReaderExec.pickSome_of_chain' depends on axioms: [propext, Quot.sound] -/
#guard_msgs in
