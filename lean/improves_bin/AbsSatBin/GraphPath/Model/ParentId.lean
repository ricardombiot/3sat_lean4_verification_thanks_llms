-- lean/improves_bin/AbsSatBin/GraphPath/Model/ParentId.lean
...
namespace AbsSatBin.GraphPath.Model.ParentId
...
theorem TL_addNode (g : GPathM) (d : NodeId) (title : String) (forb : PathNodeId → Bool)
    (_hd : d.step = g.current_step)
    (hbelow : ∀ n ∈ g.nodes, n.id.id.step < g.current_step) :
    TL (addNode g d title forb) := by
  intro n' hn' hstep
  ...
  · obtain ⟨n, hn, hEq⟩ := List.mem_map.mp hmem
    ...
    have := hbelow n hn
    have hc : (addNode g d title forb).current_step = g.current_step + 1 := rfl
    rw [hc] at hstep
    omega
...
theorem PMP_addNode (g : GPathM) (d : NodeId) (title : String) (forb : PathNodeId → Bool)
    (_htl : TL g) (h : PMP g) : PMP (addNode g d title forb) := by
  intro n' hn' p hp
  rw [addNode_nodes] at hn'
  rcases List.mem_append.mp hn' with hmem | hmem
  · obtain ⟨n, hn, hEq⟩ := List.mem_map.mp hmem
...
theorem GPMP_addNode (g : GPathM) (d : NodeId) (title : String) (forb : PathNodeId → Bool)
    (h : GPMP g) : GPMP (addNode g d title forb) := by
  refine ⟨?_, ?_⟩
  · intro n' hn' p hp
    rw [addNode_nodes] at hn'
    rcases List.mem_append.mp hn' with hmem | hmem
  ...
  · intro n' hn' hr
    ...
    · obtain ⟨pid, hpid, rfl⟩ := (mem_newRow_iff g d title forb n').mp hmem
      ...
      · exact absurd hr (parent_id_ne_none_of_mem_newRowIds g d forb pid hpos hpid)
...
theorem TL_reachable (g : GPathM) (h : Reachable reqOf forb g) : TL g := by
  induction h with
  | seed d title hstep _ => exact TL_initSeed d title hstep
  | up g d title hstep _ _ hr ih =>
    have hpr := pruned_filterAll g (reqOf d)
    show TL (up (filterAll g (reqOf d)) d title forb)
    simp only [GPathM.up]
    split
    · split
      · exact TL_of_pruned (pruned_review _) (TL_addNode _ d title forb (by rw [hpr.step_eq]; exact hstep)
            (Certifies.nodes_below_of_pruned hpr (steps_below_current reqOf forb hr)))
      · exact TL_addNode _ d title forb (by rw [hpr.step_eq]; exact hstep)
            (Certifies.nodes_below_of_pruned hpr (steps_below_current reqOf forb hr))
    · exact TL_of_pruned hpr ih
  | join g₁ g₂ hok _ _ ih₁ ih₂ => exact TL_join g₁ g₂ hok ih₁ ih₂

theorem PMP_reachable (g : GPathM) (h : Reachable reqOf forb g) : PMP g := by
  induction h with
  | seed d title _ _ => exact PMP_initSeed d title
  | up g d title _ _ _ hr ih =>
    have hpr := pruned_filterAll g (reqOf d)
    show PMP (up (filterAll g (reqOf d)) d title forb)
    simp only [GPathM.up]
    split
    · split
      · exact PMP_of_pruned (pruned_review _) (PMP_addNode _ d title forb (TL_of_pruned hpr (TL_reachable reqOf forb g hr))
            (PMP_of_pruned hpr ih))
      · exact PMP_addNode _ d title forb (TL_of_pruned hpr (TL_reachable reqOf forb g hr))
            (PMP_of_pruned hpr ih)
    · exact PMP_of_pruned hpr ih
  | join g₁ g₂ _ _ _ ih₁ ih₂ => exact PMP_join g₁ g₂ ih₁ ih₂

theorem GPMP_reachable (g : GPathM) (h : Reachable reqOf forb g) : GPMP g := by
  induction h with
  | seed d title _ _ => exact GPMP_initSeed d title
  | up g d title _ _ _ _ ih =>
    have hpr := pruned_filterAll g (reqOf d)
    show GPMP (up (filterAll g (reqOf d)) d title forb)
    simp only [GPathM.up]
    split
    · split
      · exact GPMP_of_pruned (pruned_review _) (GPMP_addNode _ d title forb (GPMP_of_pruned hpr ih))
      · exact GPMP_addNode _ d title forb (GPMP_of_pruned hpr ih)
    · exact GPMP_of_pruned hpr ih
  | join g₁ g₂ _ _ _ ih₁ ih₂ => exact GPMP_join g₁ g₂ ih₁ ih₂

theorem GPMP_filterAll (g : GPathM) (reqs : List NodeId) (h : Reachable reqOf forb g) :
    GPMP (filterAll g reqs) :=
  GPMP_of_pruned (pruned_filterAll g reqs) (GPMP_reachable reqOf forb g h)

/-- info: 'AbsSatBin.GraphPath.Model.ParentId.GPMP_reachable' depends on axioms: [propext, Quot.sound] -/
#guard_msgs in
#print axioms GPMP_reachable

theorem PMP_filterAll (g : GPathM) (reqs : List NodeId) (h : Reachable reqOf forb g) :
    PMP (filterAll g reqs) :=
  PMP_of_pruned (pruned_filterAll g reqs) (PMP_reachable reqOf forb g h)
...
/-- info: 'AbsSatBin.GraphPath.Model.ParentId.singleParent_iff_gparents' does not depend on any axioms -/
#guard_msgs in
...
/-- info: 'AbsSatBin.GraphPath.Model.ParentId.PMP_reachable' depends on axioms: [propext, Quot.sound] -/
#guard_msgs in
#print axioms PMP_reachable

/-- info: 'AbsSatBin.GraphPath.Model.ParentId.chain_eq_of_mapIds_eq' depends on axioms: [propext, Quot.sound] -/
#guard_msgs in
...
/-- info: 'AbsSatBin.GraphPath.Model.ParentId.chain_pick_mem_owners' depends on axioms: [propext, Quot.sound] -/
#guard_msgs in
...
end AbsSatBin.GraphPath.Model.ParentId
