-- lean/improves_bin/AbsSatBin/GraphPath/Model/PathExists.lean
...
theorem exists_isChain (g : GPathM) (reqs : List NodeId) (hreach : Reachable reqOf forb g)
    (hv : isValid (filterAll g reqs) = true)
    (hpos : 0 < (filterAll g reqs).current_step) :
    ∃ sel, IsChain (filterAll g reqs) sel := by
  have hs : Shape (filterAll g reqs) := Shape_filterAll reqOf forb g reqs hreach
  obtain ⟨t, ht, htstep⟩ :=
    GownersNodes.node_at_every_step reqOf forb g reqs hreach hv ((filterAll g reqs).current_step - 1)
      (by omega) (by omega)
...
/-- info: 'AbsSatBin.GraphPath.Model.PathExists.exists_isChain' depends on axioms: [propext, Quot.sound] -/
#guard_msgs in
#print axioms exists_isChain

end AbsSatBin.GraphPath.Model.PathExists
