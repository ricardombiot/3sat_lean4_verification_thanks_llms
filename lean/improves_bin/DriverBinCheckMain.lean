...
open AbsSatBin.Cnf AbsSatBin.GraphPath.Model AbsSatBin.GraphPath.Model.DriverBin
...
def checkOne (path : String) : IO Bool := do
  ...
  | .ok φ =>
    ...
    let ok := sat == truth && !dead
    IO.println s!"{name}\t{if ok then "OK" else "FAIL"}\tmachine={if sat then "SAT" else "UNSAT"}\tbrute={if truth then "SAT" else "UNSAT"}\tdead={dead}\tms={t1 - t0}"
    return ok
